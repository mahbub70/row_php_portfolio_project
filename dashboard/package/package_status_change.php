<?php 
    ...
    $status_check_query_result = mysqli_query($db_connect,"SELECT status FROM packages WHERE id=$id");
    ...
    $count_qurey_result = mysqli_query($db_connect, "SELECT * FROM packages WHERE status=1");
    ########### COUNT ACTIVE ROWS END #################

    if($status_array['status']==0){
        if(mysqli_num_rows($count_qurey_result) < 3){
            $status_update_result = mysqli_query($db_connect,"UPDATE packages SET status=1 WHERE id=$id");
            ...
            $_SESSION['limit_err'] = "You Can Active Maximum 3 Packages.";
            header("location: /packages-info");
        }
    }else{
        $status_update_result = mysqli_query($db_connect,"UPDATE packages SET status=0 WHERE id=$id");
        if($status_update_result){
