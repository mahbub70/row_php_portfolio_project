<?php 
    ...
    $status_check_query_result = mysqli_query($db_connect,"SELECT status FROM logos WHERE id=$id");
    ...
    if($status_array['status']==0){
        $deactive_all_query_result = mysqli_query($db_connect,"UPDATE logos SET status=0");

        $status_update_result = mysqli_query($db_connect,"UPDATE logos SET status=1 WHERE id=$id");
        ...
        $status_update_result = mysqli_query($db_connect,"UPDATE logos SET status=0 WHERE id=$id");
