<?php 
    ...
    $status_check_query_result = mysqli_query($db_connect,"SELECT status FROM services WHERE id=$id");
    ...
    $count_qurey_result = mysqli_query($db_connect, "SELECT * FROM services WHERE status=1");
    ########### COUNT ACTIVE ROWS END #################

    if($status_array['status']==0){
        if(mysqli_num_rows($count_qurey_result) < 6){
            $status_update_result = mysqli_query($db_connect,"UPDATE services SET status=1 WHERE id=$id");
            ...
            $_SESSION['limit_err'] = "You Can Active Maximum 6 Services.";
            header("location: /service-info");
        }
    }else{
        $status_update_result = mysqli_query($db_connect,"UPDATE services SET status=0 WHERE id=$id");
        if($status_update_result){
...
?><?php 
    ...
    // Approve Review Query Start
    $review_approve_result = mysqli_query($db_connect,"UPDATE reviews SET status=1 WHERE id=$id");
    if($review_approve_result){
